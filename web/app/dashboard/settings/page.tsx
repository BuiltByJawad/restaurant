...
export default function SettingsPage() {
  ...
  useEffect(() => {
    if (!hydrated) return;
    fetch(`/api/settings/restaurant?restaurantSlug=${restaurantSlug}`)
      .then((r) => r.json())
      .then((json) => {
        const r = json.data;
        setRestaurantData(r);
        reset({
          ...
          area: r.area ?? "",
          city: r.city ?? "Dhaka",
          minimumOrder: r.minimumOrder ?? 150,
  ...
  }, [hydrated, restaurantSlug, reset]);

  const onSave = async (data: RestaurantSettingsFormData) => {
    setSaving(true);
    try {
      const res = await fetch("/api/settings/restaurant", {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ ...data, restaurantSlug }),
      });
                ...
                <div className="p-6 lg:p-8 space-y-8">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {[
                      ...
                      { title: "Daily Reports", desc: "Get a performance report every morning", icon: Smartphone },
                    ].map((notif, i) => (
