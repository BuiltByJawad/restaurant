...
export function AdminSidebar() {
  ...
  return (
    <>
      <aside className="fixed left-0 top-0 z-30 hidden h-full w-64 flex-col bg-slate-950 lg:flex">
      ...
      <nav className="fixed inset-x-0 bottom-0 z-40 border-t border-slate-200 bg-white/95 px-2 py-2 shadow-[0_-8px_30px_rgba(15,23,42,0.08)] backdrop-blur lg:hidden">
        <div className="mx-auto grid max-w-md grid-cols-4 gap-1">
          {ADMIN_NAV.slice(0, 4).map(({ href, label, icon: Icon }) => {
            const active = pathname === href || (href !== "/admin" && pathname.startsWith(href));
            return (
              <Link
                key={href}
                href={href}
                className={cn(
                  "flex min-w-0 flex-col items-center gap-1 rounded-xl px-1.5 py-2 text-[10px] font-semibold transition-colors",
                  active ? "bg-[#0A7A5A]/10 text-[#0A7A5A]" : "text-slate-500 hover:bg-slate-50"
                )}
              >
...
export function AdminTopbar({ title, subtitle }: { title: string; subtitle?: string }) {
  return (
    <header className="min-h-16 bg-white border-b border-slate-100 flex items-center justify-between gap-3 px-4 py-3 sm:px-6">
